'''
    InsertionSort
        In this algorithm we start at index 1 then shift all elements behind until we find 
        a value that is not larger than the one we inspecting. If we do find a value that is
        less than or equal to the index value we are inspecting than we know that the last 
        index that was shifted has to be the index where the value that we were inspecting 
        belongs. 
